...
async function load(){
 ...
 const names=[...new Set([...Object.values(TrainingPlanner.catalog||{}).map(x=>x[1]),...history.map(r=>r.exercise_name).filter(Boolean)])];
 $('#exerciseHistory').replaceChildren(...names.map(name=>{const option=document.createElement('option');option.value=name;return option;}));
