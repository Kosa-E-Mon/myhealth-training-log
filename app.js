...
function rowData(id,name,amount,sets,key,unit='reps'){return {user_id:session.user.id,training_date:date,exercise_id:id,exercise_name:name,planned_reps:unit==='reps'?amount:null,planned_seconds:unit==='seconds'?amount:null,planned_sets:sets,instance_key:key};}
...
function render(){
 ...
 if(measure==='seconds'){const timerButton=document.createElement('button');timerButton.className='timer-start secondary';timerButton.textContent=`⏱ ${r.planned_seconds}秒×${r.planned_sets} タイマー`;timerButton.onclick=()=>startTimer(card.querySelector('.timer'),Number(r.planned_seconds),Number(r.planned_sets),timerButton);card.querySelector('.controls').before(timerButton);}
 ...
 const amount=Number(card.querySelector(`[data-field=${measure}]`).value),sets=Number(card.querySelector('[data-field=sets]').value),max=measure==='seconds'?3600:1000;
 if(!Number.isInteger(amount)||amount<1||amount>max||!Number.isInteger(sets)||sets<1||sets>100)throw new Error(`${unit}数1〜${max}、セット1〜100を整数で入力してください。`);
 msg('保存中…');const hour=Number(new Intl.DateTimeFormat('en-GB',{hour:'2-digit',hourCycle:'h23',timeZone:'Asia/Tokyo'}).format(new Date()));
 await request('/rest/v1/training_logs?id=eq.'+r.id+'&status=eq.planned','PATCH',{status,actual_reps:status==='completed'&&measure==='reps'?amount:null,actual_seconds:status==='completed'&&measure==='seconds'?amount:null,actual_sets:status==='completed'?sets:null,effort_level:effort,comment:note.value,completed_at:status==='completed'?new Date().toISOString():null,time_slot:hour<12?'morning':hour<18?'daytime':'evening'});
